...
// Teemo's attacking, you need to output the total time that
...
// poisoning attack will still work at time point 2, it will stop
    ...
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        for (int t : timeSeries){
            if (t >= span)
                ...
                attack += std::max(0, t + duration - span);
            span = max(span, t + duration);
        }
        return attack;
    }
};
...
public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        for (int i = 1; i < timeSeries.size(); i += 1){
            attack += std::min(timeSeries[i] - timeSeries[i - 1], duration);
        }
        attack += timeSeries.empty() ? 0 : duration;
        return attack;
    }
};

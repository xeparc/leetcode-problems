...
// 3 and 4 for its first and second level revision number.
...
// Version strings do not start or end with dots, and they
// will not be two consecutive dots.
...
int compareVersion(char * version1, char * version2)
{
    while (*version1 || *version2){
        // Find dot
        int i = 0, j = 0;
        for (; version1[i] && version1[i] != '.'; i++);
        for (; version2[j] && version2[j] != '.'; j++);
        // Check if one of the versions is exhaused
        if (i == 0 && j == 0)
            break;
        // Skip leading zeros
        for (; *version1 && *version1 == '0'; version1++, i--);
        for (; *version2 && *version2 == '0'; version2++, j--);
        if (i > j) return 1;
        if (i < j) return -1;
        // Compare
        while (i-- && j--){

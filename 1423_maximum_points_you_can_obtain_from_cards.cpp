...
// Output: 55
// Explanation: You have to take all the cards. Your score is the
...
public:
    int maxScore(vector<int>& cardPoints, int k) {
        int leftpoints = std::accumulate(cardPoints.begin(), cardPoints.begin() + k, 0);
        int rightpoints = 0;
        int maxpoints = leftpoints;
        for (int i = k - 1, j = cardPoints.size() - 1; i >= 0; i--, j--){
            rightpoints += cardPoints[j];
            leftpoints -= cardPoints[i];
            maxpoints = std::max(maxpoints, leftpoints + rightpoints);
        }
        return maxpoints;
    }
};

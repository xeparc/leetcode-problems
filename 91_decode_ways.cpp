...
// to decode it. The answer is guaranteed to fit in a 32-bit integer.
...
public:
    int numDecodings(string s) {
        ...
        // Initialization
        //
        // Can we decode the last digit alone?
        table[N - 1] = static_cast<int>(s[N - 1] != '0');
        // Can we decode the last two digits as 2-digit number?
        if (s[N - 2] != '0' and stoi(s.substr(N - 2, 2)) <= 26)
            table[N - 2] = 1;
        // Can we decode the (N - 2) digit alone?
        ...
        for (int i = N - 3; i >= 0; i--){
            if (s[i] != '0')
                table[i] += table[i + 1];
            if (s[i] != '0' and stoi(s.substr(i, 2)) <= 26)
                table[i] += table[i + 2];
        }
        return table[0];
    }

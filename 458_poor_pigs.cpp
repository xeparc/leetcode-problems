...
// What is the minimum amount of pigs you need to figure out which
...
// cool down time of m minutes. During this time, only observation
...
public:
    int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        // How many tests can we perform in minutesToTest time?
        int tests = minutesToTest / minutesToDie;
        // What's the maximum number of buckets for which we can
        // find the poisonous one with only one pig and minutesToTest time?
        int base = tests + 1;
        // How many pigs do we need for `buckets`?

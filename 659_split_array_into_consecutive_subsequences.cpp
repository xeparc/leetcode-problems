...
public:
    bool isPossible(vector<int>& nums) {
        vector<std::pair<int,int>> subsequences;
        for (int x : nums){
            std::pair<int,int>* ss = nullptr;
            for (std::pair<int,int>& subseq : subsequences){
                if (subseq.second + 1 == x){
                    if (!ss || ss->first > subseq.first){
                        ss = &subseq;
                    }
                }
            }

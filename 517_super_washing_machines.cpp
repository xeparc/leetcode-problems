...
// machines at the same time. Given an integer array machines representing
// the number of dresses in each washing machine from left to right on the line,
// return the minimum number of moves to make all the washing machines have the
...
public:
    int findMinMoves(vector<int>& machines) {
        ...
        int avg = sum / n;
        int moves = 0;
        int accumL = 0, accumR = 0, diff = 0;
        for (int i = 0; i < n; i++){
            diff = machines[i] - avg;
            accumR = 0 - accumL - diff;
            if (accumL > 0 && accumR > 0)
                moves = max(moves, max(accumL, accumR));
            else
                moves = max(relu(diff) + relu(accumL) + relu(accumR), moves);
            accumL += diff;
        }
        return moves;
    }

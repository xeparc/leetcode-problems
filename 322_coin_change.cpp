// ///////////////////////////////////////////////////////////////////////////
//
// 322. Coin Change
// https://leetcode.com/problems/coin-change/description/
// 
// ***************************************************************************
//
//
// You are given an integer array coins representing coins of
// different denominations and an integer amount representing a total amount
// of money. Return the fewest number of coins that you need to make up
// that amount.
// If that amount of money cannot be made up by any combination of the coins,
// return -1.
// You may assume that you have an infinite number of each kind of coin.
// 
// 
// Example 1:
// Input: coins = [1,2,5], amount = 11
// Output: 3
// Explanation: 11 = 5 + 5 + 1
// 
// Example 2:
// Input: coins = [2], amount = 3
// Output: -1
// 
// Example 3:
// Input: coins = [1], amount = 0
// Output: 0
// 
// Constraints:
//     * 1 <= coins.length <= 12
//     * 1 <= coins[i] <= 2^31 - 1
//     * 0 <= amount <= 10^4



#include <limits>
#include <vector>
#include <unordered_map>

using namespace std;


class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        std::sort(coins.rbegin(), coins.rend());
        memotable[0] = 0;
        return minCoinsBottomUp(coins, amount);
    }

    int minCoinsTopDown(const vector<int>& coins, const int amount) {
        if (memotable.count(amount))
            return memotable[amount];
        if (coins.back() > amount)
            return -1;

        int mincoins = std::numeric_limits<int>::max();
        for (int c : coins){
            int count = minCoinsTopDown(coins, amount - c);
            if (count != -1)
                mincoins = min(mincoins, 1 + count);
        }
        if (mincoins != std::numeric_limits<int>::max())
            memotable[amount] = mincoins;
        else
            memotable[amount] = -1;
        return mincoins == std::numeric_limits<int>::max() ? -1 : mincoins;
    }

    int minCoinsBottomUp(const vector<int>& coins, const int amount) {

        vector<int> money(amount + 1, amount + 1);
        money[0] = 0;

        for (int c : coins){
            for (int i = c; i <= amount; i++){
                money[i] = min(money[i], money[i - c] + 1);
            }
        }
        return money[amount] == amount + 1 ? -1 : money[amount];
    }

    unordered_map<int, int> memotable;
};

...
# *   void buildDict(String[] dictionary) Sets the data structure
#     with an array of distinct strings dictionary.
# *   bool search(String searchWord) Returns true if you can change
#     exactly one character in searchWord to match any string in
    ...
    def buildDict(self, dictionary: List[str]) -> None:
        for word in dictionary:
            for i in range(len(word)):
                self.data.setdefault((word[:i], word[i+1:]), []).append(word)


    def search(self, searchWord: str) -> bool:
        for i in range(len(searchWord)):
            try:
                words = self.data[(searchWord[:i], searchWord[i+1:])]
                for w in words:
                    if w != searchWord:
                        return True
            except KeyError:

...
class Solution:
    def uniquePathsIII(self, grid: List[List[int]]) -> int:
        ...
        tovisit = set((i, j) for i in M for j in N if grid[i][j] == 0 or grid[i][j] == 1)
        ...
        def backtrack(current, visited):
            ...
            visited.add(current)
            left = backtrack((i, j - 1), visited)
            right = backtrack((i, j + 1), visited)
            up = backtrack((i - 1, j), visited)
            down = backtrack((i + 1, j), visited)

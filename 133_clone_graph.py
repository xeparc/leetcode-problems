...
# second node with val = 2, and so on.
# The graph is represented in the test case using an adjacency list.

# Adjacency list is a collection of unordered lists used to represent
# a finite graph. Each list describes the set of neighbors of a
# node in the graph.
...
class Solution:
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        while Q:
            top = Q.popleft()
            x = mapping.get(top.val, Node(top.val))
            for n in top.neighbors:
                if n.val not in mapping:
                    Q.append(n)
                    mapping[n.val] = Node(n.val)
                x.neighbors.append(mapping[n.val])
            mapping[top.val] = x
